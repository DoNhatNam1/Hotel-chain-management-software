...
type HangHoa = typeof getAllHangHoaHotel[0];

export default async function AdminHangHoaTable() {
  ...
  const filteredItems = React.useMemo(() => {
    let filteredHangHoa = hangHoaData ? [...hangHoaData] : [];

    if (hasSearchFilter) {
      filteredHangHoa = filteredHangHoa.map((item) => ({
        ...item,
        GiaGocHangHoa: item.GiaGocHangHoa.toNumber(),
        GiaBanHangHoa: item.GiaBanHangHoa.toNumber()
      }));
    }
    return filteredHangHoa;
  }, [hangHoaData, filterValue]);
  ...
  const renderCell = React.useCallback((hanghoa: HangHoa, columnKey: React.Key) => {
    const cellValue = hanghoa[columnKey as keyof HangHoa];
