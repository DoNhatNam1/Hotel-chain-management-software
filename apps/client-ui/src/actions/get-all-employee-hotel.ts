...
export default async function getAllEmployeesHotel() {
    ...
    const allEmployeeHotelQuery = await prisma.tbKhachSan.findMany({
        select: {
            id: true, 
            TenKhachSan: true, 
        }
    })
    return allEmployeeHotelQuery;  // Trả về danh sách các phòng khách sạn
    }
